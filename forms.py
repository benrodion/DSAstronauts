...
class AddTransactionForm(FlaskForm):
    transaction_name = StringField('Transaction Name', validators=[DataRequired(), InputRequired(message= "Please input the transaction name"), Length(min = 1, max = 50, message = "Name should be 1 to 50 characters long")])
    amount = DecimalField('Amount', validators=[InputRequired(), NumberRange(min=0)])

    payer = SelectField('Payer', choices=[], validators=[DataRequired(), InputRequired()])
