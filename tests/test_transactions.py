...
from app.routes import app, normalize_name, update_session_names
...
def mock_db(mocker):
    ...
    mock_session = mocker.patch('app.routes.SessionLocal')
...
def test_add_transaction_route(client, mock_db, mocker):
    ...
    mocker.patch('app.routes.update_session_names', return_value=(['John', 'Alice', 'Bob'], ['John', 'Alice']))
...
def test_add_transaction_with_custom_payer(client, mock_db, mocker):
    ...
    mocker.patch('app.routes.update_session_names', return_value=(['John', 'Alice', 'Bob', 'Charlie'], ['John', 'Alice', 'Charlie']))
    response = client.post('/transactions/add', data=form_data)
